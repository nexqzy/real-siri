...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Siri Selfbot v{SELFBOT.__version__}] | Loading...')
...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            Siri.run(token, bot=False, reconnect=True)
            os.system(f'title (Siri Selfbot) - Version {SELFBOT.__version__}')
...
Siri = discord.Client()
Siri = commands.Bot(
    description='Siri Selfbot',
    command_prefix=prefix,
    self_bot=True
)
Siri.remove_command('help') 
...
async def btc_status():
    ...
    await Siri.change_presence(activity=btc_stream)
...
async def on_message_edit(before, after):
    await Siri.process_commands(after)
...
async def on_message(message):
    ...
    if f'Congratulations <@{Siri.user.id}>' in message.content:
        if giveaway_sniper == True:
            if message.author.id == 294882584201003009:    
                print(""
    ...
    if 'privnote.com' in message.content:
        if privnote_sniper == True:
            ...
            return
    await Siri.process_commands(message)
...
{Fore.CYAN}          _____                    _____                    _____                    _____          
{Fore.CYAN}         /\    \                  /\    \                  /\    \                  /\    \         
{Fore.CYAN}        /::\    \                /::\    \                /::\    \                /::\    \        
{Fore.CYAN}       /::::\    \               \:::\    \              /::::\    \               \:::\    \       
{Fore.CYAN}      /::::::\    \               \:::\    \            /::::::\    \               \:::\    \      
...
{Fore.CYAN}    /:::/__\:::\    \               \:::\    \        /:::/__\:::\    \               \:::\    \    
{Fore.CYAN}    \:::\   \:::\    \              /::::\    \      /::::\   \:::\    \              /::::\    \   
{Fore.CYAN}  ___\:::\   \:::\    \    ____    /::::::\    \    /::::::\   \:::\    \    ____    /::::::\    \  
{Fore.CYAN} /\   \:::\   \:::\    \  /\   \  /:::/\:::\    \  /:::/\:::\   \:::\____\  /\   \  /:::/\:::\    \ 
{Fore.CYAN}/::\   \:::\   \:::\____\/::\   \/:::/  \:::\____\/:::/  \:::\   \:::|    |/::\   \/:::/  \:::\____\
{Fore.CYAN}\:::\   \:::\   \::/    /\:::\  /:::/    \::/    /\::/   |::::\  /:::|____|\:::\  /:::/    \::/    /
...
{Fore.CYAN}       \::::/    /              \:::\____\               \::|   |                 \:::\____\        
{Fore.CYAN}        \::/    /                \::/    /                \:|   |                  \::/    /        
{Fore.CYAN}         \/____/                  \/____/                  \|___|                   \/____/         
{Fore.CYAN}         \/____/                  \/____/                  \|___|                   \/____/         
                                                                                                    
              
            {Fore.CYAN}Siri {SELFBOT.__version__} | {Fore.YELLOW}Logged in as: {Siri.user.name}#{Siri.user.discriminator} {Fore.CYAN}| ID: {Fore.CYAN}{Siri.user.id}   
            {Fore.CYAN}Privnote Sniper | {Fore.YELLOW}{privnote}
            {Fore.CYAN}Nitro Sniper | {Fore.YELLOW}{nitro}
            {Fore.CYAN}Giveaway Sniper | {Fore.YELLOW}{giveaway}
            {Fore.CYAN}SlotBot Sniper | {Fore.YELLOW}{slotbot}
            {Fore.CYAN}Prefix: {Fore.YELLOW}{prefix}
            {Fore.CYAN}Set | {Fore.YELLOW} Streaming Working
    '''+Fore.RESET)
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Siri Selfbot v{SELFBOT.__version__}] | Logged in as {Siri.user.name}')
    headers = {
      'Authorization': token,
...
@Siri.command()
async def id(ctx): # b'\xfc'
    await ctx.message.delete()
    await ctx.send('ID:12325968948')    
                      ...
                      module.prototype._connect = function(url) {
                        ...
                        this._handleDispatch = function(data, type) {
                          if (type === 'READY') {
                            console.log(data);
                            data.user.bot = false;
                            data.user.email = 'Siri-Was-Here@Fuckyou.com';
...
async def fox(ctx): # b'\xfc'
    ...
    em = discord.Embed(title="Siri Selfbot", color=16202876)
...
async def cmds(ctx):
    ...
    em.add_field(name="`     PURGE     `",value="`PURGE DMS`",inline=False)
    em.add_field(name="`     LOADED     `",value="`✔️`",inline=False)
    em.set_image(url="")
    em.set_footer(text="Siri SB")
    em.set_image(url="")
    em.set_thumbnail(url="https://cdn.discordapp.com/attachments/738343584515752026/748007481862783167/original_3.gif")
    await ctx.send(embed=em)
...
async def AHHH(ctx):
    await ctx.message.delete()
    em = discord.Embed(colour = discord.Colour.blue())
    em.add_field(name="`AHHHHH`",value="`SIRI ULTRA INSTINCT`",inline=False)
    em.set_image(url="")
    em.set_footer(text="HAHAA")
    em.set_image(url="https://giffiles.alphacoders.com/208/208599.gif")
    await ctx.send(embed=em)

@Siri.command()
async def dreads(ctx):
    await ctx.message.delete()
    em = discord.Embed(colour = discord.Colour.blue())
    em.add_field(name="`wats hannin`",value="`im a dredhead`",inline=False)
    em.set_image(url="")
    em.set_footer(text="cuh")
    em.set_image(url="https://cdn.discordapp.com/attachments/750201081006391386/750205537131561050/image0.jpg")
    await ctx.send(embed=em)    
@Siri.command() 
...
async def credits(ctx):
    await ctx.message.delete()
    em = discord.Embed(colour = discord.Colour.blue())
    em.add_field(name="`Credits`",value="`Devs`",inline=False)
    em.set_image(url="")
    em.set_footer(text=".")
    em.add_field(name="`Dordo&DND`",value="‎‎.",inline=False)
    await ctx.send(embed=em)    
@Siri.command()
async def nokid(ctx):
    await ctx.message.delete()
...
async def setup(ctx):
    ...
    em.add_field(name="`MASSCON`",value="`MASSCON SKYPE SIRI`",inline=False)
    em.set_image(url="")
    em.set_footer(text="Hey Siri")
    em.set_thumbnail(url="")
    await ctx.send(embed=em)
...
async def nuke(ctx):
    ...
    em.add_field(name="`WIZZ`",value="`MASS UP THE SERVER`",inline=False)
...
async def wizz(ctx):
    ...
    try:
        ...
        pass                 
    for _i in range(999):
        await ctx.guild.create_text_channel(name="SIRI wizzed")
        await ctx.guild.create_voice_channel(name="SIRI wizzed")
        await ctx.guild.create_category(name="SIRI wizzed")
        await ctx.guild.create_text_channel(name="SIRI wizzed")
        await ctx.guild.create_voice_channel(name="SIRI wizzed")
        await ctx.guild.create_category(name="SIRI wizzed")
        await ctx.guild.create_text_channel(name="SIRI wizzed")
...
@Siri.command(name='ebay-view', aliases=['ebay-view-bot', 'ebayviewbot', 'ebayview'])
...
@Siri.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Siri.command(aliases=['pfp', 'avatar'])
...
@Siri.command(name='1337-speak', aliases=['1337speak'])
...
async def blank(ctx): # b'\xfc'
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
          try:      
             await Siri.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
          except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Siri.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
              await Siri.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)

@Siri.command(name='set-pfp', aliases=['setpfp', 'pfpset'])
...
@Siri.command(aliases=['dong', 'penis'])
...
@Siri.command(aliases=['changehypesquad'])
...
@Siri.command(aliases=['tokenfucker', 'disable', 'crash']) 
...
@Siri.command(aliases=['fakeconnection', 'spoofconnection'])
...
@Siri.command(aliases=['tokinfo', 'tdox'])
...
async def copy(ctx): # b'\xfc'
    await ctx.message.delete()
    await Siri.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Siri.guilds:
...
async def destroy(ctx): # b'\xfc'
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="https://SIRI.HEHE",
            reason="https://docs.google.com/document/d/15oK88fb_YCDNdKURTtv6m5clA911jrRraxyzDKpCUr0/edit?pli=1",
            icon=None,
            banner=None
        )  
    except:
...
@Siri.command()
async def raid(ctx, amount: int, *, message): # b'\xfc'
    await ctx.message.delete()    
    for _i in range(amount):
        await ctx.send(RAID)        
...
async def dm(ctx, user : discord.Member, *, message): # b'\xfc'
    await ctx.message.delete()
    user = Siri.get_user(user.id)
    if ctx.author.id == Siri.user.id:
        return
    else:
        try:
            await user.send(message) 
...
@Siri.command(name='get-color', aliases=['color', 'colour', 'sc'])
...
@Siri.command(aliases=['rainbow-role'])
...
@Siri.command(aliases=['slots', 'bet'])
...
@Siri.command(name='auto-bump', aliases=['bump'])
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            count += 1 
            channel = Siri.get_channel(int(channelid))
...
@Siri.command(name='backup-f', aliases=['friendbackup', 'friend-backup', 'backup-friends', 'backupf'])
async def _backup_f(ctx): # b'\xfc'
    await ctx.message.delete()
    for friend in Siri.user.friends:
       friendlist = (friend.name)+'#'+(friend.discriminator)   
       with open('Backup/Friends.txt', 'a+') as f:
           f.write(friendlist+"\n" )
    for block in Siri.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )

@Siri.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Siri.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Siri.command()
async def repor(ctx, user: discord.Member): # b'\xfc'
    await ctx.message.delete()
    r = requests.get("https://cdn.discordapp.com/attachments/750170615071244329/750191481691570206/reported-typographic-stamp-sign-label-black-rubber-stamp-series-eps-vectors_csp59880931.png")
    res = r.json()
    em = discord.Embed(description=user.mention)
    em.set_image(url=res['url'])
    await ctx.send(embed=em)
...
async def purge(ctx, amount: int): # b'\xfc'
    await ctx.message.delete()    
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Siri.user).map(lambda m: m):
        try:
...
@Siri.command(name='group-leaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
async def _group_leaver(ctx): # b'\xfc'
    await ctx.message.delete()
    for channel in Siri.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()

@Siri.command()
async def report(ctx): # b'\xfc'
    await ctx.message.delete()
...
@Siri.command()
async def set(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    stream = discord.Streaming(
        name=message,
        url=stream_url, 
    )
    await Siri.change_presence(activity=stream)    
...
async def game(ctx, *, message): # b'\xfc'
    ...
    await Siri.change_presence(activity=game)

@Siri.command()
async def listening(ctx, *, message): # b'\xfc'
    await ctx.message.delete()
    await Siri.change_presence(
...
@Siri.command(aliases=['markasread', 'ack'])
async def read(ctx): # b'\xfc'
    await ctx.message.delete()
    for guild in Siri.guilds:
        await guild.ack()
...
@Siri.command(name='role-hexcode', aliases=['rolecolor'])
async def _role_hexcode(ctx, *, role: discord.Role): # b'\xfc'
    await ctx.message.delete()
    await ctx.send(f"{role.name} : {role.color}")

@Siri.command(name='get-hwid', aliases=['hwid', 'gethwid', 'hwidget'])
...
@Siri.command(aliases=['btc-stream', 'streambtc', 'stream-btc', 'btcstatus'])
async def btcstream(ctx):  # b'\xfc'
    await ctx.message.delete()   
    btc_status.start()        

@Siri.command(name='steal-all-pfp', aliases=['steal-all-pfps', 'stealallpfps'])
async def _steal_all_pfp(ctx): # b'\xfc'
    await ctx.message.delete()
    Dump(ctx)

@Siri.command(aliases=['clearconsole', 'consoleclear'])
...
@Siri.command(name='gmail-bomb', aliases=['gmail-bomber', 'gmailbomb', 'email-bomber', 'emailbomber'])
